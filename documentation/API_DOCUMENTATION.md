...
API keys are generated by administrators through the admin endpoint:

**Endpoint**: `POST /api/v1/admin/generate-key`

**Authentication**: Requires admin login (session-based)
...
- Keys are associated with a specific feedlot
- Keys can be deactivated without deletion
- Each API key usage is logged with timestamp
